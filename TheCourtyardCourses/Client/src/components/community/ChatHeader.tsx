...
const ChatHeader = ({ community }: ChatHeaderProps) => {
  ...
  return (
    <header className="flex items-center gap-3 w-full border-b-2 border-border bg-surface px-4 py-3 shrink-0">
      <button
        onClick={() => navigate(-1)}
        className="p-2 rounded-sm border-2 border-border text-text hover:bg-bg hover:border-accent-hover transition-colors shrink-0 group"
        aria-label="Go back"
        title="Go back"
      >
      ...
      >
        {thumbUrl ? (
          <img
            src={thumbUrl}
            alt={community.name}
            className="w-11 h-11 rounded-sm object-cover border-2 border-border shadow-[2px_2px_0_var(--color-border)] shrink-0"
          />
        ) : (
          <div className="w-11 h-11 rounded-sm bg-accent/20 flex items-center justify-center border-2 border-border shadow-[2px_2px_0_var(--color-border)] shrink-0">
            <UsersThreeIcon size={22} weight="fill" className="text-accent" />
          </div>
        )}

        <div className="flex-1 min-w-0">
          <h3 className="font-heading text-base text-text m-0 truncate leading-tight">
            {community.name}
          </h3>
          <div className="flex items-center gap-1.5 text-[10px] text-text-muted font-heading uppercase tracking-wider">
            {community.isPrivate && (
      ...
      {isTeacher && (
        <button
          onClick={openDetails}
          className="p-2 rounded-sm border-2 border-border text-text hover:bg-bg hover:border-accent-hover hover:text-accent-hover transition-colors shrink-0"
